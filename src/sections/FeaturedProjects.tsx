        ...
        <div ref={headerRef} className="flex flex-col md:flex-row md:items-end md:justify-between mb-16 md:mb-20 opacity-0">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl lg:text-5xl font-display text-[#1a1a1a] tracking-tight">
              {featuredProjectsConfig.titleRegular} <span className="font-serif italic font-normal text-[#c9a86c]">{featuredProjectsConfig.titleItalic}</span>
            </h2>
          </div>
          {featuredProjectsConfig.viewAllText && (
            <a
              href={featuredProjectsConfig.viewAllHref || '#contact'}
              className="mt-6 md:mt-0 inline-flex items-center gap-2 text-[#4a4a4a] hover:text-[#c9a86c] font-sans text-sm transition-colors duration-300 group"
            >
              {featuredProjectsConfig.viewAllText}
            ...
            >
              ...
              >
                <div className="aspect-[4/3] overflow-hidden">
                  <img
                    src={project.image.replace('.jpg', '-640w.webp')}
                    srcSet={`${project.image.replace('.jpg', '-320w.webp')} 320w, ${project.image.replace('.jpg', '-640w.webp')} 640w, ${project.image.replace('.jpg', '-960w.webp')} 960w`}
                ...
                <div className="absolute inset-0 pointer-events-none">
                  <div className="absolute top-4 left-4 w-10 h-10 border-t-2 border-l-2 border-[#c9a86c]/70 transition-all duration-300 group-hover:w-12 group-hover:h-12 group-hover:border-[#c9a86c]" />
                  <div className="absolute top-4 right-4 w-10 h-10 border-t-2 border-r-2 border-[#c9a86c]/70 transition-all duration-300 group-hover:w-12 group-hover:h-12 group-hover:border-[#c9a86c]" />
                  <div className="absolute bottom-4 left-4 w-10 h-10 border-b-2 border-l-2 border-[#c9a86c]/70 transition-all duration-300 group-hover:w-12 group-hover:h-12 group-hover:border-[#c9a86c]" />
                  <div className="absolute bottom-4 right-4 w-10 h-10 border-b-2 border-r-2 border-[#c9a86c]/70 transition-all duration-300 group-hover:w-12 group-hover:h-12 group-hover:border-[#c9a86c]" />
                  {/* Center crosshair */}
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="w-px h-6 bg-[#c9a86c]/50" />
                    <div className="w-6 h-px bg-[#c9a86c]/50 -mt-3 -ml-[11px]" />
                  </div>
                </div>
              ...
              <div className={`project-content opacity-0 ${index % 2 === 1 ? 'md:order-1 md:text-right' : ''}`}>
                <div className={`project-text-item flex items-center gap-3 mb-4 ${index % 2 === 1 ? 'md:justify-end' : ''}`}>
                  <span className="text-[#c9a86c] font-sans text-sm">{project.category}</span>
                  <span className="w-1 h-1 rounded-full bg-[#c9a86c]/50" />
                  <span className="text-[#4a4a4a] font-sans text-sm">{project.year}</span>
                </div>
                <h3 className="project-text-item text-2xl md:text-3xl lg:text-4xl font-display text-[#1a1a1a] tracking-tight mb-4">
                  {project.title}
                </h3>
                <p className="project-text-item text-[#4a4a4a] font-sans text-base md:text-lg leading-relaxed mb-6">
                  {project.description}
                </p>
                {featuredProjectsConfig.viewProjectText && (
                  <a
                    href="#contact"
                    className={`project-text-item inline-flex items-center gap-2 text-[#1a1a1a] font-sans text-sm border-b border-[#c9a86c]/50 pb-1 hover:border-[#c9a86c] transition-colors duration-300 group/link ${
                      index % 2 === 1 ? 'md:flex-row-reverse' : ''
                    }`}
                  >
                    {featuredProjectsConfig.viewProjectText}
