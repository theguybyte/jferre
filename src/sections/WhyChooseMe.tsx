      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div ref={headerRef} className="text-center mb-16 md:mb-20 opacity-0">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-display text-[#1a1a1a] tracking-tight">
            {whyChooseMeConfig.titleRegular} <span className="font-serif italic font-normal text-[#c9a86c]">{whyChooseMeConfig.titleItalic}</span>
          </h2>
        </div>
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-6 md:gap-8">
          ...
          {whyChooseMeConfig.featureCards.map((card, index) => (
            <div key={index} className="feature-card-image opacity-0 group">
              <div className="relative aspect-[3/4] rounded-lg overflow-hidden bg-[#1a1a1a]">
                <img
                  src={card.image.replace('.jpg', '-640w.webp')}
                  srcSet={`${card.image.replace('.jpg', '-320w.webp')} 320w, ${card.image.replace('.jpg', '-640w.webp')} 640w, ${card.image.replace('.jpg', '-960w.webp')} 960w`}
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <p className="text-white/90 font-display font-semibold text-lg mb-2">
                    {card.title}
                  </p>
          ...
          {whyChooseMeConfig.stats.length > 0 && (
            <div
              ref={statsRef}
              className="feature-card-stats opacity-0 bg-white rounded-lg p-8 md:p-10 flex flex-col justify-between border border-[#e8e0d5]"
            >
              <div>
                {whyChooseMeConfig.statsLabel && (
                  <p className="text-[#c9a86c] text-xs font-sans uppercase tracking-[3px] mb-8">
                    {whyChooseMeConfig.statsLabel}
                  </p>
                )}
                <div className="space-y-8">
                  {whyChooseMeConfig.stats.map((stat, index) => (
                    <div key={index} className="border-b border-[#e8e0d5] pb-6 last:border-0">
                      <p className="text-4xl md:text-5xl font-display text-[#c9a86c] tracking-tight">
                        <Counter
                          end={stat.value}
                          suffix={stat.suffix}
                      ...
                      <p className="text-[#4a4a4a] font-sans text-sm mt-1">
                        {stat.label}
                      </p>
                    </div>
                  ))}
        ...
        {whyChooseMeConfig.wideImage && (
          <div ref={wideRef} className="mt-16 md:mt-24 relative rounded-lg overflow-hidden group opacity-0">
            <div className="aspect-[16/9] md:aspect-[3/1] overflow-hidden">
              <img
                src={whyChooseMeConfig.wideImage.replace('.jpg', '-960w.webp')}
                srcSet={`${whyChooseMeConfig.wideImage.replace('.jpg', '-640w.webp')} 640w, ${whyChooseMeConfig.wideImage.replace('.jpg', '-960w.webp')} 960w, ${whyChooseMeConfig.wideImage.replace('.jpg', '-1280w.webp')} 1280w`}
                sizes="100vw"
            ...
            {(whyChooseMeConfig.wideTitle || whyChooseMeConfig.wideDescription) && (
              <div className="wide-text-overlay absolute bottom-8 left-8 md:bottom-12 md:left-12 max-w-md opacity-0">
                {whyChooseMeConfig.wideTitle && (
                  <p className="text-white/90 font-display font-semibold text-2xl md:text-3xl mb-3">
                    {whyChooseMeConfig.wideTitle}
                  </p>
                )}
